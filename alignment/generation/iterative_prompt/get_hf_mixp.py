...
elif "iterative-prompt-v1-iter6" in script_args.dataset_name_or_path:
    ds1 = load_dataset("RLHFlow/iterative-prompt-v1-iter6-20K", split="train")
    ds2 = load_dataset("RLHFlow/iterative-prompt-v1-iter7-20K", split="train")
    ds3 = load_dataset("RLHFlow/iterative-prompt-v1-iter8-20K", split="train")
    ds4 = load_dataset("RLHFlow/iterative-prompt-v1-iter9-20K", split="train")
    ds = concatenate_datasets([ds1, ds2, ds3, ds4])
else: 
...
assert K % 2 == 0
per_K = K//2
fusionmodel_1 = FusionModel(models=[model, ref_model], weights=[1.5, -0.5])
fusionmodel_2 = FusionModel(models=[model, ref_model], weights=[0.5, 0.5])
for input_ids, attention_masks in tqdm(dataloader):
    res = fusionmodel_1.generate(input_ids=input_ids, attention_mask=attention_masks, max_length=input_ids.shape[1]+script_args.max_new_tokens, do_sample=True, temperature=script_args.temperature, top_p=1.0, num_return_sequences=per_K, pad_token_id=tokenizer.pad_token_id)
    ...
    res = fusionmodel_2.generate(input_ids=input_ids, attention_mask=attention_masks, max_length=input_ids.shape[1]+script_args.max_new_tokens, do_sample=True, temperature=script_args.temperature, top_p=1.0, num_return_sequences=per_K, pad_token_id=tokenizer.pad_token_id)
    for idx in range(0, res.shape[0]//per_K):
        for jdx in range(0, per_K):
            text = tokenizer.decode(res[idx*per_K+jdx], skip_special_tokens=True)
